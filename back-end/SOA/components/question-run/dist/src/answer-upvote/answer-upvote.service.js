...
let AnswerUpvoteService = class AnswerUpvoteService {
    ...
    async create(req, createAnswerUpvoteDto) {
        ...
        return async_calls_1.createAnswerUpvote(createAnswerUpvoteDto)
    ...
    async remove(req, id) {
        ...
        const upvote = await async_calls_1.getOneAnswerUpvote(params);
        if (!upvote.data) {
        ...
        return async_calls_1.deleteAnswerUpvote(id)
            .then(response => { return response.data; })
            .catch(err => { throw new common_1.BadRequestException(err.response.data.message); });
    }
};
